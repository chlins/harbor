...
func PushMiddleware() func(http.Handler) http.Handler {
	before := middleware.BeforeRequest(func(r *http.Request) error {
		return loadWasmHooks(r.Context(), wasmhook.HookInjectBeforePush)
	})

	after := middleware.AfterResponse(func(w http.ResponseWriter, r *http.Request, statusCode int) error {
		return loadWasmHooks(r.Context(), wasmhook.HookInjectAfterPush)
	})
