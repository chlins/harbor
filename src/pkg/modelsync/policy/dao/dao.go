...
// DAO defines the DAO operations of model sync policy
type DAO interface {
	...
	// List the model sync policies according to the query
...
func (d *dao) Get(ctx context.Context, id int64) (*model.Policy, error) {
	...
	if err := ormer.Read(policy); err != nil {
		if e := orm.AsNotFoundError(err, "model sync policy %d not found", id); e != nil {
			err = e
		}
		return nil, err
	}
...
func (d *dao) Create(ctx context.Context, policy *model.Policy) (int64, error) {
	...
	if e := orm.AsConflictError(err, "model sync policy %s already exists", policy.Name); e != nil {
		err = e
	}
	return id, err
}
...
func (d *dao) Update(ctx context.Context, policy *model.Policy, props ...string) error {
	...
	if n == 0 {
		return errors.NotFoundError(nil).WithMessagef("model sync policy %d not found", policy.ID)
	}
	return nil
}
...
func (d *dao) Delete(ctx context.Context, id int64) error {
	...
	if n == 0 {
		return errors.NotFoundError(nil).WithMessagef("model sync policy %d not found", id)
	}
	return nil
}
